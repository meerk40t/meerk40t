...
Provides and registers the GRBL driver for sefrocut.

The GRBL module implements comprehensive support for GRBL (G-code based laser controller) devices, enabling SefroCut to communicate with a wide variety of CNC laser cutters and engravers. This module follows the standard SefroCut device driver pattern with device service, driver, and controller components for robust G-code generation and hardware control.
...
- **`driver.py`** - GRBLDriver implementation
  - Translates SefroCut cutcode operations to G-code commands
...
- **`plugin.py`** - Device registration and lifecycle management
  - Registers multiple GRBL device variants with SefroCut kernel
...
- **SefroCut Projects**: Native `.mk` files with GRBL export
...
This module provides SefroCut with comprehensive GRBL controller support, enabling reliable communication with a wide variety of G-code based laser systems.
