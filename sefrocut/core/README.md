...
├── exceptions.py           # SefroCut-specific exception hierarchy
...
The central orchestrator that registers all core services with the SefroCut kernel:
...
High-performance geometric processing library (9003 lines) providing the foundation for all geometric operations in SefroCut:
...
```python
# Register help URIs
kernel.register("webhelp/tutorial", "https://sefrocut.org/tutorial")
```
...
```python
from sefrocut.core.elements import Elemental
from sefrocut.core.planner import Planner
...
```python
from sefrocut.core.drivers import Driver
...
```python
from sefrocut.core.parameters import validate_parameter
...
This core module provides the foundation for SefroCut's laser cutting capabilities, implementing a sophisticated pipeline from design to hardware control with extensive optimization and error handling features.
