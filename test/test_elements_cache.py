...
class TestElementsCache(unittest.TestCase):
    ...
    def test_cache_invalidated_on_add(self):
        ...
        self.elements._invalidate_elems_cache()
        for i in range(3):
        ...
        # Normal add should invalidate cache via node_attached
        self.elements.elem_branch.add(type="elem path")
    ...
    def test_cache_invalidated_on_fast_remove(self):
        ...
        # clear_elements(fast=True) ends with notify_tree_structure_changed
        # which calls structure_changed -> _invalidate_elems_cache
        self.elements.clear_elements(fast=True)
        self.assertIsNone(self.elements._elems_cache)
    ...
    def test_ops_cache_invalidated_on_clear(self):
        ...
        self.elements._invalidate_ops_cache()
        ...
        # clear_operations(fast=True) ends with notify_tree_structure_changed
        # which calls structure_changed -> _invalidate_ops_cache
        self.elements.clear_operations(fast=True)
    ...
    def test_append_children_fast_invalidates_cache(self):
        ...
        _ = list(self.elements.elems())
        self.assertIsNotNone(self.elements._elems_cache)
        # append_children(fast=True) ends with notify_tree_structure_changed
        # which calls structure_changed -> _invalidate_elems_cache
