...
def draw(segments, min_x, min_y, max_x, max_y, filename="test.png"):
    ...
    im = Image.new("RGBA", (int(max_x - min_x), int(max_y - min_y)), "white")
    ...
    for i in range(len(segments) - 1):
        ...
        if f is None or t is None:
            continue
        draw.line(((f.real - min_x, f.imag - min_y), (t.real - min_x, t.imag - min_y)), fill="#000000")
...
class TestGeomstr(unittest.TestCase):
    ...
    def test_geomstr_hatch(self):
        ...
        hatch = Geomstr.hatch(gs, distance=400, angle=0)
        hatch.uscale(0.005)
        print(hatch)
        bounds = hatch.bbox()
