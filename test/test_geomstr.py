...
class TestGeomstr(unittest.TestCase):
    ...
    def test_bbox_vectorized_performance(self):
        """Test the performance of bbox() vectorized optimization across different sizes."""
        print("\n--- Testing bbox() vectorized performance ---")
        
        # Test different geometry complexities
        test_sizes = [5, 10, 15, 20, 30, 50, 75, 100]
        
        for n in test_sizes:
            geom = Geomstr()
            
            # Create a complex shape with mixed segment types
            points = []
            for i in range(n):
                # Create various geometric patterns
                angle = 2 * 3.14159 * i / n
                radius = 50 + 20 * __import__('math').sin(3 * angle)
            ...
            try:
                ...
                vectorized = "✓ VECTORIZED" if geom.index > 10 else "Standard"
