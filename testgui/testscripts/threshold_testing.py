...
try:
    from meerk40t.tools.geomstr import (
        Geomstr, stitch_geometries, stitcheable_nodes,
        THRESHOLD_STITCHEABLE_NODES, THRESHOLD_STITCH_GEOMETRIES, 
        THRESHOLD_CLOSE_GAPS, THRESHOLD_BBOX, THRESHOLD_LENGTH
    )
...
class ComprehensiveThresholdAnalyzer:
    ...
    def test_bbox_threshold(self) -> List[Tuple[int, float, bool]]:
        ...
        sizes = [current-10, current-5, current-2, current, current+2, current+5, current+10]
    ...
    def test_length_threshold(self) -> List[Tuple[int, float, bool]]:
        ...
        sizes = [current-15, current-10, current-5, current, current+5, current+10, current+15]
    ...
    def test_stitch_geometries_threshold(self) -> List[Tuple[int, float, bool]]:
        ...
        sizes = [current-15, current-10, current-5, current, current+5, current+10]
        ...
        for n in sizes:
            ...
            avg_time = self.time_function(lambda: stitch_geometries(geometries, tolerance=1.0), 3)
    ...
    def test_stitcheable_nodes_threshold(self) -> List[Tuple[int, float, bool]]:
        ...
        sizes = [current-10, current-5, current, current+5, current+10, current+15]
        ...
        for n in sizes:
            ...
            avg_time = self.time_function(lambda: stitcheable_nodes(nodes, tolerance=1.0), 3)
    ...
    def analyze_performance_data(self, results: List[Tuple[int, float, bool]], 
                               current_threshold: int, function_name: str) -> int:
        """Analyze performance data and recommend optimal threshold."""
        if len(results) < 4:
            return current_threshold
        
        standard_data = [(size, time) for size, time, vectorized in results 
                        if not vectorized and time != float('inf')]
        vectorized_data = [(size, time) for size, time, vectorized in results 
        ...
        if avg_vectorized > avg_standard * 1.2:  # 20% overhead tolerance
            ...
            if self.verbose:
                ...
                print(f"   ⚠️  OVERHEAD: {overhead:.1f}% - RAISE threshold to {recommendation}")
            return recommendation
        elif avg_vectorized > avg_standard * 1.05:  # 5% overhead tolerance
            recommendation = current_threshold + 5
            if self.verbose:
                overhead = ((avg_vectorized / avg_standard) - 1) * 100
                print(f"   ⚠️  MINOR OVERHEAD: {overhead:.1f}% - RAISE threshold to {recommendation}")
            return recommendation
        else:
            if self.verbose:
    ...
    def quick_test(self):
        ...
        length_rec = self.analyze_performance_data(length_results, THRESHOLD_LENGTH, "length")
    ...
    def full_analysis(self):
        ...
        functions_to_test = [
            ...
            ('stitch_geometries', self.test_stitch_geometries_threshold, THRESHOLD_STITCH_GEOMETRIES),
            ('stitcheable_nodes', self.test_stitcheable_nodes_threshold, THRESHOLD_STITCHEABLE_NODES),
        ]
        ...
        for func_name, test_func, current_threshold in functions_to_test:
            ...
            try:
                results = test_func()
                recommended = self.analyze_performance_data(results, current_threshold, func_name)
    ...
    def generate_final_report(self, recommendations: Dict[str, Any]):
        ...
        for func_name, current in self.current_thresholds.items():
            ...
            print(f"{func_name:18s}: {current:2d} → {recommended:2d} | Line {line:4d} | {status}")
...
Examples:
  ...
  python threshold_testing.py --validate
    ...
    parser.add_argument('--validate', action='store_true',
                       help='Validate current threshold settings')
