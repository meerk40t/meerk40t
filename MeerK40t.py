...
def on_rotate_popup(element, value):
    ...
    def specific(event):
        center = element.center
        for e in element.flat_elements(types=(PathElement)):
            e.matrix.post_rotate(value, center[0], center[1])
        project("elements", 0)
...
def menu_subpath(element):
    def specific(event):
        context = element
        es = element.all_children_of_type(types=(PathElement))
        for e in es:
            e.detach()
            p = Path()
            svg_parser.parse_svg_path(p, e.path)
            add = []
