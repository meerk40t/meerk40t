    ...
    def write(self, index=0, packet=None):
        ...
        if packet is not None:
            ...
            if b == GetSerialNo:
                ...
                footpedal_pin = getattr(device, "footpedal_pin", 15)  # Default to pin 15 if not set
                pinvalue = 0
                try:
                    import wx
                    ...
                    pass
                self.set_implied_response(b'\x00\x00' + struct.pack("<H", pinvalue) + b'\x00\x00\x00\x00')
            if self.send:
