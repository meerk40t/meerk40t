...
Governs the generic commands issued by laserjob and spooler and converts that into regular Marlin Gcode output.
"""
...
class MarlinDriver(Parameters):
    ...
    def __repr__(self):
        return f"MarlinDriver({self.name})"
    ...
    def plot_start(self):
        ...
        for q in self.queue:
            ...
            if isinstance(q, LineCut):
                ...
                # Marlin has no core GPIO functionality
                pass
    ...
    def blob(self, data_type, data):
        ...
        if data_type != "marlin":
            return
        data_blob = bytes.decode(data, "latin-1")
        for split in data_blob.split("\r"):
            g = split.strip()
            if g:
    ...
    def set_power_scale(self, factor):
        # Marlin can only deal with factors between 10% and 200%
