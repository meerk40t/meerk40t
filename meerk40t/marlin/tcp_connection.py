...
Communicate with a TCP network destination with the Marlin driver.
"""
...
class TCPOutput:
    ...
    def connect(self):
        try:
            ...
            self.disconnect()
            self.service.signal("marlin;status", "timeout connect")
        except ConnectionError:
            self.disconnect()
            self.service.signal("marlin;status", "connection error")
        except socket.gaierror as e:
            self.disconnect()
            self.service.signal("marlin;status", "address resolve error")
        except socket.herror as e:
            self.disconnect()
            self.service.signal("marlin;status", f"herror: {str(e)}")
        except OSError as e:
            self.disconnect()
            self.service.signal("marlin;status", f"Host down {str(e)}")
