...
{self._get_html_body(command_result, device_controls, spooler_html, console_output)}
        ...
        for device in available_devices:
            ...
            for idx, e in enumerate(spooler.queue):
                ...
                try:
                    ...
                    row_data["steps"] = f"{spool_obj.steps_done}/{spool_obj.steps_total}"
                except AttributeError:
                    pass
                ...
                try:
                    ...
                    row_data["runtime"] = f"{int(hours)}:{str(int(minutes)).zfill(2)}:{str(int(seconds)).zfill(2)}"
                except AttributeError:
                    pass
                ...
                try:
                    ...
                    if isinf(t):
                        ...
                        hours, remainder = divmod(t, 3600)
                        minutes, seconds = divmod(remainder, 60)
                        row_data["estimate"] = f"{int(hours)}:{str(int(minutes)).zfill(2)}:{str(int(seconds)).zfill(2)}"
                except AttributeError:
                    pass
    ...
    def run_server(self) -> None:
        ...
        try:
            ...
            self.events_channel(_("Could not start web server: {error}").format(error=str(e)))
        except Exception as e:
