...
class PlotCut(CutObject):
    ...
    def __init__(self, parameter_object=None, passes=1):
        CutObject.__init__(self, parameter_object=parameter_object, passes=passes)
        self.plot = []
        self.max_dx = None
        self.max_dy = None
        self.min_x = None
    ...
    def check_if_rasterable(self):
        ...
        po = self.parameter_object
        po._raster_alt = False
        po._constant_move_x = False
        po._constant_move_y = False
        po.raster_step = 0
        if po.speed < 80:
            # Twitchless gets sketchy at 80.
            po._force_twitchless = True
            return False
        ...
        if 0 < self.max_dx <= 15:
            self.v_raster = True
            po._constant_move_y = True
        if 0 < self.max_dy <= 15:
            self.h_raster = True
            po._constant_move_x = True
        # if self.vertical_raster or self.horizontal_raster:
        po.raster_step = min(self.max_dx, self.max_dy)
        po._raster_alt = True
        return True
