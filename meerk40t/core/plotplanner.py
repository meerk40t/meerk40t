...
def single(single_default=1):
    ...
    while True:
        ...
        x = event[0]
        y = event[1]
        if single_x is None or single_y is None:
            # Our single_x or single_y position is not established.
            single_x = x
            single_y = y
        on = event[2] if len(event) >= 3 else single_default
...
def apply_ppi():
    ...
    while True:
        ...
        if event is None:
            continue
        if isinstance(event, LaserSettings):
            settings = event
            continue
        x, y, on = event
        ...
        # PPI is always on.
        ppi_total += settings.power * on
        if on and dot_left > 0:
            # Process remaining dot_length, must be on or partially on.
            dot_left -= 1
            on = 1
        else:
            ...
            if ppi_total >= 1000.0:
                # PPI >= 1000: triggers on.
                on = 1
                ppi_total -= 1000.0 * settings.dot_length
                dot_left = settings.dot_length - 1
            else:
                # PPI < 1000: triggers off.
                on = 0
        yield x, y, on
...
def shift():
        ...
        while True:
            ...
            if isinstance(event, bool):
                ...
                continue
            x, y, on = event
            if (x is None or y is None) or not (force_shift or settings.shift_enabled):
                # If we have an established buffer, flush the buffer.
                while len(shift_buffer) > 0:
                    shift_pixels <<= 1
                    bx, by = shift_buffer.pop()
                    bon = (shift_pixels >> 3) & 1
                    yield bx, by, bon
                # Yield the current event.
                yield x, y, on
                continue
            ...
            shift_buffer.insert(0, (x, y))
            if shift_pixels == 0b0101:
                shift_pixels = 0b0011
            elif shift_pixels == 0b1010:
                shift_pixels = 0b1100

            if len(shift_buffer) >= 4:
                # When buffer is full start popping off values.
                bx, by = shift_buffer.pop()
                bon = (shift_pixels >> 3) & 1
                yield bx, by, bon
...
def group():
    ...
    group_enabled = True  # Grouped Output Required for Lhymicro-gl.
    group_x = None
    group_y = None
    group_on = None
    group_dx = 0
