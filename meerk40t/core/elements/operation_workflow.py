...
class OperationNode:
    ...
    def __post_init__(self):
        if hasattr(self.operation, 'label'):
            self.label = self.operation.label or f"{self.operation_type.replace('op ', '').title()}"
        else:
...
class OperationWorkflow:
    ...
    def _calculate_combined_bbox(self, elements: List[Any]) -> Tuple[float, float, float, float]:
    ...
    def _optimize_groups_by_travel(self, groups: List[WorkflowGroup]) -> List[WorkflowGroup]:
        """
    ...
    def get_workflow_summary(self) -> Dict[str, Any]:
        ...
        total_travel = sum(group.total_travel_distance for group in self.workflow_groups)
