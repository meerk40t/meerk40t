    ...
    def offset_path(path, offset_value=0, radial_connector=False, linearize=True, interpolation=500):
        ...
        def stitch_segments_at_index(stitchpath, index, orgintersect, radial=False, closed=False):
            ...
            if idx2 >= lp:
                ...
                idx2 = idx2 % lp
                while not isinstance(stitchpath._segments[idx2], (Arc, Line, QuadraticBezier, CubicBezier)):
                    idx2 += 1
                ...
                if isinstance(seg2, Line):
                    ...
                    if p is not None:
                        ...
                        if 0 <= s <= 1 and 0 <= t <= 1:
                            ...
                            seg1.end = Point(p)
                            seg2.start = Point(p)
                            if idx2>0 and isinstance(stitchpath._segments[idx2-1], Move):
            ...
            if needs_connector and seg1.end != seg1.start:
                if radial:
                    ...
                    connect_seg = Arc(start=startpt, end=endpt, center=Point(orgintersect), ccw=ccw)
                else:
        ...
        for subpath in path.as_subpaths():
            ...
            for idx in range(len(p._segments) - 1, -1, -1):
                ...
                if isinstance(segment, Close):
                    ...
                    idx1 = idx
                    while (idx1 >= 0) and not isinstance(p._segments[idx1], (Arc, Line, QuadraticBezier, CubicBezier)):
                        idx1 -= 1
                    idx2 = 0
                    while (idx2 < len(p)) and not isinstance(p._segments[idx2], (Arc, Line, QuadraticBezier, CubicBezier)):
                        idx2 += 1
                    segment = Line(Point(p._segments[idx1].end), Point(p._segments[idx2].start))
