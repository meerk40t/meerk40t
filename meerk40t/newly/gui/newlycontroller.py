...
class NewlyInformationPanel(wx.ScrolledWindow):
    ...
    **End-User Perspective:**
    ...
    def __init__(self, *args, context=None, **kwargs):
        ...
        self.lbl_connection_status = wx.StaticText(self, wx.ID_ANY, _("Connection Status:"))
        ...
        self.txt_connection_type = wx.StaticText(self, wx.ID_ANY, _("None"))
        ...
        self.txt_machine_index = wx.StaticText(self, wx.ID_ANY, _("0"))

        self.lbl_current_position = wx.StaticText(self, wx.ID_ANY, _("Current Position:"))
        self.txt_current_position = wx.StaticText(self, wx.ID_ANY, _("0.0, 0.0"))

        self.lbl_current_mode = wx.StaticText(self, wx.ID_ANY, _("Current Mode:"))
        self.txt_current_mode = wx.StaticText(self, wx.ID_ANY, _("init"))
        ...
        self.txt_relative_mode = wx.StaticText(self, wx.ID_ANY, _("True"))
        ...
        self.lbl_connection_details = wx.StaticText(self, wx.ID_ANY, _("Connection Details:"))
        self.txt_connection_details = wx.StaticText(self, wx.ID_ANY, _("N/A"))

        self.lbl_device_enumeration = wx.StaticText(self, wx.ID_ANY, _("Device Enumeration:"))
        ...
        self.lbl_controller_status = wx.StaticText(self, wx.ID_ANY, _("Controller Status:"))
    ...
    def __set_properties(self):
        ...
        labels = [
            self.lbl_connection_status, self.lbl_connection_type, self.lbl_device_info,
            self.lbl_current_position, self.lbl_current_mode, self.lbl_current_speed,
            self.lbl_current_power, self.lbl_relative_mode
        ]
        for label in labels:
            label.SetFont(info_font)

        values = [
            self.txt_connection_status, self.txt_connection_type, self.txt_device_info,
            self.txt_current_position, self.txt_current_mode, self.txt_current_speed,
            self.txt_current_power, self.txt_relative_mode
        ]
        for value in values:
    ...
    def __do_layout(self):
        ...
        sizer_connection = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Connection")), wx.VERTICAL)
        ...
        # Device info section
        sizer_device = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Device")), wx.VERTICAL)
        ...
        sizer_position = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Position & Mode")), wx.VERTICAL)
        ...
        # Settings section
        sizer_settings = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Settings")), wx.VERTICAL)
        ...
        sizer_buffer = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Command Buffer")), wx.VERTICAL)
        ...
        sizer_details = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Connection Details")), wx.VERTICAL)
        ...
        sizer_enumeration = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Device Enumeration")), wx.VERTICAL)
        ...
        sizer_backend = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Backend Status")), wx.VERTICAL)
        ...
        sizer_controller = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Controller Status")), wx.VERTICAL)
        ...
        sizer_mode = wx.StaticBoxSizer(wx.StaticBox(self, wx.ID_ANY, _("Mode Details")), wx.VERTICAL)
    ...
    def update_information(self):
        ...
        try:
            ...
            # Connection status
            connected = controller.connected if hasattr(controller, 'connected') else False
            self.txt_connection_status.SetLabel(_("Connected") if connected else _("Disconnected"))

            # Connection type
            self.txt_connection_type.SetLabel(controller.status if hasattr(controller, 'status') else _("Unknown"))   
            ...
            if hasattr(controller, '_last_x') and hasattr(controller, '_last_y'):
                ...
                self.txt_current_position.SetLabel(f"{Length(pos_x, digits=1).length_mm}, {Length(pos_y, digits=1).length_mm}")
            else:
            ...
            if hasattr(controller, '_set_speed'):
                ...
                self.txt_current_speed.SetLabel(str(speed) if speed is not None else _("N/A"))
            else:
            ...
            if hasattr(controller, '_set_power'):
                ...
                self.txt_current_power.SetLabel(str(power) if power is not None else _("N/A"))
            else:
            ...
            if hasattr(controller, '_pwm_frequency'):
                ...
                self.txt_pwm_frequency.SetLabel(str(pwm_freq) if pwm_freq is not None else _("N/A"))
            else:
                    ...
                    for idx, device in controller.devices.items():
                        if device:
                            try:
                                ...
                                enumeration.append(f"Dev {idx}: Bus {bus}, Addr {address}")
            ...
            if hasattr(controller, 'is_shutdown'):
                controller_info.append(f"Shutdown: {getattr(controller, 'is_shutdown', False)}")

            if hasattr(controller, '_realtime'):
                controller_info.append(f"Realtime: {getattr(controller, '_realtime', False)}")

            if hasattr(controller, 'paused'):
                controller_info.append(f"Paused: {getattr(controller, 'paused', False)}")

            if hasattr(controller, '_disable_connect'):
                controller_info.append(f"Auto-connect disabled: {getattr(controller, '_disable_connect', False)}")

            if hasattr(controller, '_abort_open'):
                controller_info.append(f"Abort requested: {getattr(controller, '_abort_open', False)}")
            ...
            if hasattr(controller, '_pwm_frequency') and controller._pwm_frequency is not None:
...
class NewlyController(MWindow):
    def __init__(self, *args, **kwds):
        super().__init__(800, 600, *args, **kwds)  # Increased size for side-by-side layout
        
        # Create main horizontal sizer
        main_sizer = wx.BoxSizer(wx.HORIZONTAL)
        
        # Create controller panel (left side)
        self.controller_panel = NewlyControllerPanel(self, wx.ID_ANY, context=self.context)
        ...
        main_sizer.Add(self.info_panel, 0, wx.EXPAND | wx.ALL, 5)  # 0 proportion so it takes minimum space
