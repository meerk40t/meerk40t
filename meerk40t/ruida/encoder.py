...
class RuidaEncoder:
    ...
    def axis_x_move(self, x):
        self(AXIS_X_MOVE, encode32(x))

    def axis_z_move(self, z):
        self(AXIS_Z_MOVE, encode32(z))

    def axis_a_move(self, a):
        self(AXIS_A_MOVE, encode32(a))

    def axis_u_move(self, u):
        self(AXIS_U_MOVE, encode32(u))

    def move_abs_xy(self, x, y):
        self(MOVE_ABS_XY, encode_coord(x), encode_coord(y))

    def move_rel_xy(self, dx, dy):
        self(MOVE_REL_XY, encode_relcoord(dx), encode_relcoord(dy))

    def move_rel_x(self, dx):
        self(MOVE_REL_X, encode_relcoord(dx))

    def move_rel_y(self, dy):
        self(MOVE_REL_Y, encode_relcoord(dy))

    def cut_abs_xy(self, x, y):
        self(CUT_ABS_XY, encode_coord(x), encode_coord(y))

    def cut_rel_xy(self, dx, dy):
        self(CUT_REL_XY, encode_relcoord(dx), encode_relcoord(dy))

    def cut_rel_x(self, dx):
        self(CUT_REL_X, encode_relcoord(dx))

    def cut_rel_y(self, dy):
        self(CUT_REL_Y, encode_relcoord(dy))

    def imd_power_1(self, power):
        self(IMD_POWER_1, encode_power(power))

    def imd_power_2(self, power):
        self(IMD_POWER_2, encode_power(power))

    def imd_power_3(self, power):
        self(IMD_POWER_3, encode_power(power))

    def imd_power_4(self, power):
        self(IMD_POWER_4, encode_power(power))

    def end_power_1(self, power):
        self(END_POWER_1, encode_power(power))

    def end_power_2(self, power):
        self(END_POWER_2, encode_power(power))

    def end_power_3(self, power):
        self(END_POWER_3, encode_power(power))

    def end_power_4(self, power):
        self(END_POWER_4, encode_power(power))

    def min_power_1(self, power):
        self(MIN_POWER_1, encode_power(power))

    def max_power_1(self, power):
        self(MAX_POWER_1, encode_power(power))

    def min_power_2(self, power):
        self(MIN_POWER_2, encode_power(power))

    def max_power_2(self, power):
        self(MAX_POWER_2, encode_power(power))

    def min_power_3(self, power):
        self(MIN_POWER_3, encode_power(power))

    def max_power_3(self, power):
        self(MAX_POWER_3, encode_power(power))

    def min_power_4(self, power):
        self(MIN_POWER_4, encode_power(power))

    def max_power_4(self, power):
        self(MAX_POWER_4, encode_power(power))

    def laser_interval(self, time):
        self(LASER_INTERVAL, encode_time(time))

    def add_delay(self, time):
        self(ADD_DELAY, encode_time(time))

    def laser_on_delay(self, time):
        self(LASER_ON_DELAY, encode_time(time))

    def laser_off_delay(self, time):
        self(LASER_OFF_DELAY, encode_time(time))

    def laser_on_delay_2(self, time):
        self(LASER_ON_DELAY2, encode_time(time))

    def laser_off_delay_2(self, time):
        self(LASER_OFF_DELAY2, encode_time(time))

    def min_power_1_part(self, part, power):
        self(MIN_POWER_1_PART, encode_part(part), encode_power(power))

    def max_power_1_part(self, part, power):
        self(MAX_POWER_1_PART, encode_part(part), encode_power(power))

    def min_power_2_part(self, part, power):
        self(MIN_POWER_2_PART, encode_part(part), encode_power(power))

    def max_power_2_part(self, part, power):
        self(MAX_POWER_2_PART, encode_part(part), encode_power(power))

    def min_power_3_part(self, part, power):
        self(MIN_POWER_3_PART, encode_part(part), encode_power(power))

    def max_power_3_part(self, part, power):
        self(MAX_POWER_3_PART, encode_part(part), encode_power(power))

    def min_power_4_part(self, part, power):
        self(MIN_POWER_4_PART, encode_part(part), encode_power(power))

    def max_power_4_part(self, part, power):
        self(MAX_POWER_4_PART, encode_part(part), encode_power(power))

    def through_power_1(self, power):
        self(THROUGH_POWER_1, encode_power(power))

    def through_power_2(self, power):
        self(THROUGH_POWER_2, encode_power(power))

    def through_power_3(self, power):
        self(THROUGH_POWER_3, encode_power(power))

    def through_power_4(self, power):
        self(THROUGH_POWER_4, encode_power(power))

    def frequency_part(self, laser, part, frequency):
        self(FREQUENCY_PART, encode_index(laser), encode_part(part), encode_frequency(frequency))

    def speed_laser_1(self, speed):
        self(SPEED_LASER_1, encode_speed(speed))

    def speed_axis(self, speed):
        self(SPEED_AXIS, encode_speed(speed))

    def speed_laser_1_part(self, part, speed):
        self(SPEED_LASER_1_PART, encode_part(part), encode_speed(speed))

    def force_eng_speed(self, speed):
        self(FORCE_ENG_SPEED, encode_speed(speed))

    def speed_axis_move(self, speed):
        self(SPEED_AXIS_MOVE, encode_speed(speed))
    ...
    def layer_number_part(self, part):
        self(LAYER_NUMBER_PART, encode_part(part))

    def en_laser_tube_start(self):
        self(EN_LASER_TUBE_START)

    def x_sign_map(self, value):
        self(X_SIGN_MAP, encode_index(value))

    def layer_color(self, color):
        self(LAYER_COLOR, encode_color(color))

    def layer_color_part(self, part, color):
        self(LAYER_COLOR, encode_part(part), encode_color(color))
    ...
    def en_ex_io(self, value):
        ...
        @return:
        """
        self(EN_EX_IO, encode_index(value))

    def max_layer_part(self, part):
        self(MAX_LAYER_PART, encode_part(part))

    def u_file_id(self, file_number):
        self(MAX_LAYER_PART, encode_file_number(file_number))

    def zu_map(self, value):
        self(ZU_MAP, encode_index(value))

    def work_mode_part(self, part, mode):
        self(WORK_MODE_PART, encode_part(part), encode_index(mode))
    ...
    def rapid_move_x(self, x, light=False, origin=False):
        self(RAPID_MOVE_X, self._rapid_options(light=light, origin=origin), encode_coord(x))

    def rapid_move_y(self, y, light=False, origin=False):
        self(RAPID_MOVE_Y, self._rapid_options(light=light, origin=origin), encode_coord(y))

    def rapid_move_z(self, z, light=False, origin=False):
        self(RAPID_MOVE_Z, self._rapid_options(light=light, origin=origin), encode_coord(z))

    def rapid_move_u(self, u, light=False, origin=False):
        self(RAPID_MOVE_U, self._rapid_options(light=light, origin=origin), encode_coord(u))

    def rapid_move_xy(self, x, y, light=False, origin=False):
        self(RAPID_MOVE_XY, self._rapid_options(light=light, origin=origin), encode_coord(x), encode_coord(y))

    def rapid_move_xyu(self, x, y, u, light=False, origin=False):
        self(RAPID_MOVE_XYU, self._rapid_options(light=light, origin=origin), encode_coord(x), encode_coord(y), encode_coord(u))

    def rapid_feed_axis(self, light=False, origin=False):
        self(RAPID_FEED_AXIS_MOVE, self._rapid_options(light=light, origin=origin))

    def get_setting(self, mem):
        self(GET_SETTING, encode_mem(mem), real=True)

    def set_setting(self, mem, value):
        self(SET_SETTING, encode_mem(mem), encode_value(value), encode_value(value))

    def document_file_upload(self, file_number, value, value1):
        self(DOCUMENT_FILE_UPLOAD, encode_value(value), encode_value(value1))

    def document_file_end(self):
        self(DOCUMENT_FILE_END)

    def set_file_sum(self, value):
        self(SET_FILE_SUM, encode_value(value))
    ...
    def set_filename(self, filename):
        self(SET_FILENAME, bytes(filename[:9], encoding="utf-8"), b"\x00")

    def process_top_left(self, top, left):
        self(PROCESS_TOP_LEFT, encode_coord(top), encode_coord(left))

    def process_repeat(self, v0, v1, v2, v3, v4, v5, v6):
        self(PROCESS_REPEAT, encode14(v0), encode14(v1), encode14(v2), encode14(v3), encode14(v4), encode14(v5), encode14(v6))

    def array_direction(self, direction):
        self(ARRAY_DIRECTION, encode_index(direction))

    def feed_repeat(self, value, value1):
        self(FEED_REPEAT, encode32(value), encode32(value1))

    def process_bottom_right(self, bottom, right):
        self(PROCESS_BOTTOM_RIGHT, encode_coord(bottom), encode_coord(right))

    def array_repeat(self, v0, v1, v2, v3, v4, v5, v6):
        self(ARRAY_REPEAT, encode14(v0), encode14(v1), encode14(v2), encode14(v3), encode14(v4), encode14(v5), encode14(v6))

    def feed_length(self, length):
        self(FEED_LENGTH, encode32(length))

    def feed_info(self, value):
        self(FEED_INFO, encode_value(value))

    def array_en_mirror_cut(self, index):
        self(ARRAY_EN_MIRROR_CUT, encode_index(index))

    def array_min_point(self, min_x, min_y):
        self(ARRAY_MIN_POINT, encode_coord(min_x), encode_coord(min_y))

    def array_max_point(self, max_x, max_y):
        self(ARRAY_MAX_POINT, encode_coord(max_x), encode_coord(max_y))

    def array_add(self, x, y):
        self(ARRAY_ADD, encode_coord(x), encode_coord(y))

    def array_mirror(self, mirror):
        self(ARRAY_MIRROR, encode_index(mirror))

    def block_x_size(self, x0, x1):
        self(BLOCK_X_SIZE, encode_coord(x0), encode_coord(x1))

    def by_test(self):
        self(BY_TEST, encode32(0x11227766))

    def array_even_distance(self, value):
        self(ARRAY_EVEN_DISTANCE, encode_value(value))

    def set_feed_auto_pause(self, index):
        self(SET_FEED_AUTO_PAUSE, encode_index(index))

    def union_block_property(self):
        self(UNION_BLOCK_PROPERTY)

    def document_min_point(self, min_x, min_y):
        self(DOCUMENT_MIN_POINT, encode_coord(min_x), encode_coord(min_y))

    def document_max_point(self, max_x, max_y):
        self(DOCUMENT_MAX_POINT, encode_coord(max_x), encode_coord(max_y))

    def part_min_point(self, part, min_x, min_y):
        self(PART_MIN_POINT, encode_part(part), encode_coord(min_x), encode_coord(min_y))

    def part_max_point(self, part, max_x, max_y):
        self(PART_MAX_POINT, encode_part(part), encode_coord(max_x), encode_coord(max_y))

    def pen_offset(self, axis, coord):
        self(PEN_OFFSET, encode_index(axis), encode_coord(coord))

    def layer_offset(self, axis, coord):
        self(LAYER_OFFSET, encode_index(axis), encode_coord(coord))

    def set_current_element_index(self, index):
        self(SET_CURRENT_ELEMENT_INDEX, encode_index(index))

    def part_min_point_ex(self, part, min_x, min_y):
        self(PART_MIN_POINT_EX, encode_part(part), encode_coord(min_x), encode_coord(min_y))

    def part_max_point_ex(self, part, max_x, max_y):
        self(PART_MAX_POINT_EX, encode_part(part), encode_coord(max_x), encode_coord(max_y))

    def array_start(self, index):
        self(ARRAY_START, encode_index(index))
    ...
    def element_max_index(self, index):
        self(ELEMENT_MAX_INDEX, encode_index(index))

    def element_name_max_index(self, index):
        self(ELEMENT_NAME_MAX_INDEX, encode_index(index))

    def enable_block_cutting(self, enable):
        self(ENABLE_BLOCK_CUTTING, encode_index(enable))

    def display_offset(self, dx, dy):
        self(DISPLAY_OFFSET, encode_coord(dx), encode_coord(dy))

    def feed_auto_calc(self, enable):
        self(FEED_AUTO_CALC, encode_index(enable))

    def element_index(self, index):
        self(ELEMENT_INDEX, encode_index(index))

    def element_name(self, name):
        self(ELEMENT_NAME, bytes(name[:9], encoding="utf-8"), b"\x00")

    def element_array_min_point(self, x, y):
        self(ELEMENT_ARRAY_MIN_POINT, encode_coord(x), encode_coord(y))

    def element_array_max_point(self, x, y):
        self(ELEMENT_ARRAY_MAX_POINT, encode_coord(x), encode_coord(y))

    def element_array(self, v0, v1, v2, v3, v4, v5, v6):
        self(ELEMENT_ARRAY, encode14(v0), encode14(v1), encode14(v2), encode14(v3), encode14(v4), encode14(v5), encode14(v6))

    def element_array_add(self, x, y):
        self(ELEMENT_ARRAY_ADD, encode_coord(x), encode_coord(y))

    def element_array_mirror(self, mirror):
        self(ELEMENT_ARRAY_MIRROR, encode_index(mirror))
