...
class OperationProperty(MWindow):
    def __init__(self, *args, node=None, **kwds):
        ...
        self.notebook_main.AddPage(param_panel, "Properties")
        param_panel.set_widgets(node)
        self.add_module_delegate(param_panel)
        self.panels.append(param_panel)

        self.add_module_delegate(param_panel.raster_panel.panel_start)

        for panel in self.context.lookup_all("operationproperty"):
            page_panel = panel(
                self.notebook_main, wx.ID_ANY, context=self.context, node=node
            )
