...
class Themes(Service):
    ...
    def __init__(self, kernel, index=None, *args, **kwargs):
        Service.__init__(
            self, kernel, "themes" if index is None else f"themes{index}"
        )
    ...
    def load_system_default(self):
        ...
        tp["start_bg_inactive"] = wx.Colour("DARK SLATE GREY")
        tp["start_fg_focus"] = wx.BLACK
        # Stop button
        tp["stop_bg"] = wx.Colour(127, 0, 0)
        tp["stop_fg"] = wx.Colour("WHITE") 
        tp["stop_fg_focus"] = wx.BLACK

        tp["arm_bg"] = wx.Colour(0, 127, 0)
        tp["arm_bg_inactive"] = wx.Colour("MAROON") 
