...
class SpeedPpiPanel(wx.Panel):
    ...
    def set_widgets(self, node):
        self.operation = node
        if self.operation.settings.get("speed",0) is not None:
            self.text_speed.SetValue(str(self.operation.settings.get("speed",0)))
        if self.operation.settings.get("power",0) is not None:
            self.update_power_label()
            self.text_power.SetValue(str(self.operation.settings.get("power",0)))
    ...
    def update_power_label(self):
        # if self.operation.settings.get("power",0) <= 100:
...
class PassesPanel(wx.Panel):
    ...
    def set_widgets(self, node):
        self.operation = node
        if self.operation.settings.get("passes_custom",0) is not None:
            self.check_passes.SetValue(self.operation.settings.get("passes_custom",0))
        if self.operation.settings.get("passes",0) is not None:
            self.text_passes.SetValue(str(self.operation.settings.get("passes",0)))
...
class PanelStartPreference(wx.Panel):
    ...
    def set_widgets(self, node):
        self.operation = node
        if self.operation.settings.get("raster_preference_top") is not None:
            self.slider_top.SetValue(self.operation.settings.get("raster_preference_top") + 1)
        if self.operation.settings.get("raster_preference_left") is not None:
            self.slider_left.SetValue(
                self.operation.settings.get("raster_preference_left") + 1
            )
        if self.operation.settings.get("raster_preference_right") is not None:
            self.slider_right.SetValue(
                self.operation.settings.get("raster_preference_right") + 1
            )
        if self.operation.settings.get("raster_preference_bottom") is not None:
            self.slider_bottom.SetValue(
                self.operation.settings.get("raster_preference_bottom") + 1
            )
    ...
    def calculate_raster_lines(self):
        ...
        last = None
        direction = self.operation.settings.get("raster_direction")
        r_start = list()
        r_end = list()
        t_start = list()
        t_end = list()
        ...
        d_end = list()
        factor = 3
        unidirectional = self.operation.settings.get("raster_swing")
        ...
        if direction == 0 or direction == 1 or direction == 4:
            ...
            if direction == 1:
                # Bottom to Top
                if self.operation.settings.get("raster_preference_bottom") > 0:
                ...
                step = -self.operation.settings.get("raster_step") * factor
            else:
                # Top to Bottom or Crosshatch
                if self.operation.settings.get("raster_preference_top") > 0:
                ...
                step = self.operation.settings.get("raster_step",2) * factor
            if step == 0:
                step = 1
        ...
        if direction == 2 or direction == 3 or direction == 4:
            ...
            if direction == 2:
                # Right to Left
                if self.operation.settings.get("raster_preference_right",0) > 0:
                ...
                step = -self.operation.settings.get("raster_step",0) * factor
            else:
                # Left to Right or Crosshatch
                if self.operation.settings.get("raster_preference_left") > 0:
...
class RasterSettingsPanel(wx.Panel):
    ...
    def set_widgets(self, node):
        ...
        if self.operation.settings.get("raster_swing") is not None:
            self.radio_directional_raster.SetSelection(
                self.operation.settings.get("raster_swing",0)
            )
    ...
    def on_combo_raster_direction(self, event=None):
        ...
        self.context.raster_direction = self.operation.settings.get("raster_direction")
        self.context.elements.signal("element_property_reload", self.operation)
