...
class RotationWidget(Widget):
    ...
    def process_draw(self, gc):
        if not self.visible or self.master.tool_running:  # We don't need that overhead
            return
...
class ReferenceWidget(Widget):
    ...
    def process_draw(self, gc):
        if not self.visible or self.master.tool_running:
...
class SelectionWidget(Widget):
    ...
    def init(self, context):
        ...
        context.listen("element_property_update", self.invalidate_widgets)
        context.listen("modified_by_tool", self.invalidate_widgets)
    ...
    def final(self, context):
        ...
        context.unlisten("element_property_update", self.invalidate_widgets)
        context.unlisten("modified_by_tool", self.invalidate_widgets)
    ...
    def invalidate_widgets(self, origin=None, *args):
        """
        Forces widget rebuild on next draw by clearing bounds cache.
        """
        self._last_bounds = None
    ...
    def process_draw(self, gc):
        ...
        if not self.visible or (self.scene.context.draw_mode & DRAW_MODE_SELECTION != 0):
            return
        if self.scene.pane.suppress_selection:
            return
        ...
        # Check if we have any selected elements first
        bounds = elements.selected_area()
        if bounds is None:
            self.clear()
            return
            
        # Only rebuild if we have selection changes or significant bounds changes
        need_rebuild = (
            len(self) == 0 or  # No widgets yet
            hasattr(self, '_last_bounds') and self._last_bounds != bounds or  # Bounds changed
            not hasattr(self, '_last_bounds')  # First time
        )
        ...
        if need_rebuild:
            ...
            # Set handle usage preferences (cached from context)
            try:
            ...
            # Analyze selected elements for capabilities and properties
            self.is_ref = False
            self.single_element = True
            is_locked = True
            no_scale = True
            ...
            # Calculate widget sizes and determine what to show
            msize = 5 * self.line_width
            rotsize = 3 * msize
            show_skew_x = self.use_handle_skew
            show_skew_y = self.use_handle_skew
            
            # Check if there's enough room for skew handles
            ...
            # Just update the graphics context and pen for existing widgets
            if hasattr(self, 'font_size') and hasattr(self, '_cached_font'):
                gc.SetFont(self._cached_font, self.scene.colors.color_manipulation)
                gc.SetPen(self.selection_pen)
